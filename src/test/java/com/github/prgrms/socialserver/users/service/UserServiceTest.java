...
class UserServiceTest {
  ...
  @BeforeAll
  void setUp() {
    email = new EmailVO("test@gmail.com");
    password = "1234";
  }
  ...
  @Order(1)
  void 사용자를_추가한다() {
    UserEntity user = userService.join(email, password);
  ...
  @Order(2)
  void 사용자를_이메일로_조회한다() {
    UserEntity user = Optional.ofNullable(userService.findByEmail(email)).orElse(null);
    assertThat(user, is(notNullValue()));
    assertThat(user.getEmail(), is(email));
  ...
  @Order(5)
  void 친구_목록을_가져온다() {
    List<ConnectedUserEntity> connected = userService.findAllConnectedUser(IdVO.of(UserEntity.class, 1L));
  ...
  @Order(6)
  void 친구ID_목록을_가져온다() {
    List<IdVO<UserEntity, Long>> connectedIds = userService.findConnectedIds(IdVO.of(UserEntity.class, 1L));
